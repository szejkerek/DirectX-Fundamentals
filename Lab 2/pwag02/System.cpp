...
void System::OnMouseDown(WPARAM btnState, int x, int y)
{
	m_lastMousePos.x = x;
	m_lastMousePos.y = y;
	m_renderWidget->UpdateViewProjectionCBuffer();
	SetCapture(m_hwnd);
}
...
void System::OnMouseMove(WPARAM btnState, int x, int y)
{
	if ((btnState & MK_LBUTTON) != 0)
	{
		...
		m_renderWidget->Camera.Theta += dx;
		m_renderWidget->Camera.Phi += dy;
	...
	else if ((btnState & MK_RBUTTON) != 0)
	{
		...
		m_renderWidget->Camera.Radius += dx - dy;
	}
...
void System::OnKeyUp()
{
	m_renderWidget->Position.Y += 0.1f;
	m_renderWidget->UpdateWorldCBuffer();
}
...
void System::OnKeyLeft()
{
	m_renderWidget->Position.X -= 0.1f;
	m_renderWidget->UpdateWorldCBuffer();
}

void System::OnKeyRight()
{
	m_renderWidget->Position.X += 0.1f;
	m_renderWidget->UpdateWorldCBuffer();
}
